...
class ContrastiveLoss(nn.Module):
    ...
    def class_contrastive_loss(self, z_i, z_j, predicted_cls):
        ...
        cls_similarity_matrix = torch.isclose(torch.bitwise_xor(cls_representations, cls_representations.T), torch.tensor([0]))

        positives = (cls_similarity_matrix.float() - torch.eye(cls_similarity_matrix.size(0))) * similarity_matrix
...
def get_consistency_loss(weakly_augmented_outputs, strongly_augmented_outputs, threshold):
    ...
    return F.cross_entropy(strongly_augmented_outputs[mask], targets_u[mask])